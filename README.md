...
本 fork 面向 iOS 手机 Agent 的工程化使用体验做了增强，重点不是替换原模型能力，而是提升部署、诊断、交互和运行稳定性。

面向求职/作品集的完整讲法见：[BetterGLM 求职展示文档](docs/betterglm_portfolio.md)。

已增加：

- `Doctor` 一键诊断：检查 Python、依赖、环境变量、设备工具、WDA 可达性和模型 API。
- iOS WiFi-only 支持：WDA 可访问时，不再因为 `idevice_id` 查不到 USB 设备而误退出。
- CLI 中文交互增强：优先使用 `prompt_toolkit`，支持更好的输入编辑和历史记录。
- 模型动作解析失败重试：模型没有输出 `do(...)` / `finish(...)` 时，不再误判任务完成。
- `.env` 配置加载：支持把 API Key、模型、WDA 地址等放入本地配置文件。
- Agent Loop Guard：App 不存在、动作执行异常或连续重复动作时快速失败，不再一直卡到最大步数。
- iOS App 别名归一：兼容 `safari`、`系统设置`、`Settings`、`luckincoffee` 等模型常见输出。
- 日志回放：按步骤保存截图、模型思考、动作、执行结果和耗时，方便复盘 Agent 失败原因。
- 本地 Web 控制台：提供任务输入、Doctor 诊断、运行状态和回放预览，方便面试或作品集演示。
- Web 手动停止：任务卡住或存在误操作风险时，可在控制台请求停止并把回放标记为 `cancelled`。
- WDA 快速失败：iOS WDA 断连时直接标记 `wda_error`，不把黑屏 fallback 继续交给模型误判。
- 历史 Dashboard：汇总 Web 回放的评分通过率、平均分、平均步数和失败类型，方便展示 Agent 质量趋势。
- 任务模板：把常用演示任务沉淀成可复现模板，支持变量覆盖和自定义 JSON 模板文件。
- 任务评分器：根据模板成功条件和回放证据输出 passed/failed、分数和检查项。
- 点击精度审计：记录模型坐标、截图像素、WDA 窗口坐标和最终触控坐标，便于排查“点不准”问题。

诊断命令：

```bash
python main.py --doctor --device-type ios

# 只检查本机和设备环境，跳过模型 API 网络检查
python main.py --doctor --device-type ios --doctor-skip-model
```

安装为可执行命令后也可以运行：

```bash
betterglm --doctor --device-type ios
```

开启日志回放：

```bash
python main.py --device-type ios --replay-dir runs "打开 Safari 搜索天气"
```

任务结束后会输出：

```text
Replay: runs/<run-id>/index.html
```

回放目录会包含 `metadata.json`、`steps.json`、每一步截图和可直接打开的 `index.html`。截图可能包含手机隐私信息，默认不会开启，也不会提交到 git。
...
然后打开 `http://127.0.0.1:8765`。控制台默认把运行记录保存到 `runs`，并在首页展示历史 Dashboard：总任务数、已评分任务、评分通过率、未评分完成数、平均分、平均步数和每条回放入口。运行中的任务可以点击“停止任务”请求中断，Agent 会在下一次安全检查点退出，并把回放状态标记为 `cancelled`。历史 Dashboard 会递归扫描 `runs` 下的回放，所以 `runs/web`、`runs/smoke-test` 和 `runs/benchmark-*` 都能汇总展示。它适合在面试里展示：你不仅能让 Agent 执行任务，还能解释每一步为什么执行、失败在哪里、历史质量指标如何变化。
...
模板的价值是把“我临时输入了一个任务”升级成“我有一组可复现的 Agent 场景库”。这更适合求职展示：能说明你关注评测、复盘和产品化交付，而不是只跑通了一次 demo。

内置模板覆盖 Safari、设置、备忘录，以及微信、支付宝、淘宝、京东、美团、大众点评、小红书、抖音、哔哩哔哩、知乎、微博、高德地图、携程、得物、什么值得买、Luckin Coffee 等常见 App。模板默认选择搜索、查看、停留结果页这类低风险任务，避免下单、支付、发消息、点赞、评论等敏感动作。

对已有回放重新评分：

```bash
python ios.py --template ios_safari_weather \
  --template-var city=北京 \
  --evaluate-replay runs/smoke-test/20260629-143116-打开-Safari-搜索北京天气
```

评分器会读取 `metadata.json` 和 `steps.json`，输出 `evaluation.json`，并把评分摘要写回回放元数据。它默认检查任务是否完成、步骤中是否有错误、是否超过最大步骤数、最终 App 是否匹配，以及回放证据里是否包含模板要求的关键词。关键词检查不会把原始用户任务当作成功证据，避免“任务里写了天气，所以评分通过”这类虚高。
...
- 执行器会把模型坐标映射到截图像素，并在 iOS 上结合 WDA `window/size` 做窗口坐标校准。
- 每次 Tap/Swipe/Double Tap/Long Press 都会把坐标审计信息写入 `action_result.metadata`。
- 回放 HTML 会展示 `Coordinate Audit`，用于定位是模型点偏、截图缩放偏，还是 WDA 坐标换算偏。
