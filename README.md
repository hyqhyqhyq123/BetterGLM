...
然后打开 `http://127.0.0.1:8765`。控制台默认把运行记录保存到 `runs/web`，并在首页展示历史 Dashboard：总任务数、已评分任务、评分通过率、未评分完成数、平均分、平均步数和每条回放入口。它适合在面试里展示：你不仅能让 Agent 执行任务，还能解释每一步为什么执行、失败在哪里、历史质量指标如何变化。
