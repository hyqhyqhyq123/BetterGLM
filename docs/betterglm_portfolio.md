...
| Agent Loop Guard | App 未安装或模型重复同一动作时会卡到最大步数 | 失败快停、循环检测、真实场景鲁棒性 |
| iOS App 别名归一 | 模型常输出 safari、系统设置等非标准 App 名 | 工程兼容性、真实设备适配 |
| 日志回放 | 任务失败后不知道哪一步出错 | 可观测性、问题复盘、质量评估 |
| Web 控制台 | 命令行不适合面试演示和非技术用户试用 | 产品化包装、前后端集成、异步任务状态 |
| Web 手动停止 | App 未安装、模型卡住或任务有风险时缺少人工刹车 | Agent 可控性、真实设备安全边界 |
| WDA 快速失败 | iOS 连接断开时黑屏 fallback 被模型误判成敏感页面 | 设备链路诊断、坏输入隔离 |
...
关键词证据会排除原始用户任务和模型思考长文，优先使用执行结果、结构化动作和屏幕/步骤证据，避免分数被 prompt 本身污染。
...
真实手机任务里经常会出现“模型以为 App 存在，但设备没有安装”的情况。比如用户要求打开 Luckin Coffee，但手机上没有这个 App，模型可能会在系统搜索页反复点击、返回、重新搜索，直到耗尽 `max_steps`。

BetterGLM 增加了 `AgentLoopGuard`：

- `App not found` / `not installed` 这类启动失败会立即结束任务，并把回放状态写为 `failed`
- 连续 3 次输出完全相同的动作会触发 `repeated_action_loop`
- 执行动作异常不再伪装成 completed，而是作为失败写入 replay
- 历史 Dashboard 会把这类失败归因为 `app_not_installed` 或 `repeated_action_loop`

这件事在面试里可以这样讲：

> 我没有让 Agent 在错误状态里盲目跑满最大步数，而是在执行 loop 里增加了失败快停和重复动作检测。这样 App 未安装、动作异常、重复 Back/Tap 这类真实场景问题会更快暴露，并且能在回放和 Dashboard 中形成稳定的失败类型。
